...
La visualización de datos es un recurso útil a la hora de comunicar y pensar estrategias de desarrollo. En materia de política túristica, como en muchas otrás áreas de la administración pública, trabajar con datos geográficos resulta de vital importancia para la planificación y promoción.
...
El paquete [shiny](https://shiny.rstudio.com/) ofrece una oportunidad para un desarrollo de estas características, al permitir construir una aplicación con una interfaz accesible (UI) y un trasfondo donde se procese la información geográfica (server).

## MapeAr

**mapeAr** surge en este contexto como un recurso de la Dirección Nacional de Mercados y Estadística (DNMyE) disponible en: https://tableros.yvera.tur.ar/mapeAr/

La plataforma consiste de dos pestañas, una con información sobre cómo utilizar la aplicación y otra con los controles y visualización para poder realizar un mapa. 

```{r out.width="80%", out.height= "30%"}
knitr::include_graphics("imagenes/mapear.png")
```

Por _default_ la shiny carga una capa con la geometría de Argentina y su división política (provincias). Esta es la denominada **capa base**, sobre la cual se pueden hacer algunas modificaciones (filtrar provincias específicas o agregar las etiquetas con los nombres, por ejemplo). 

Utilizando la lógica de [ggplot2](https://ggplot2.tidyverse.org/), la aplicación permite incorporar más capas de datos georeferenciados sobre la capa base. En la medida que se realicen incorporaciones o ajustes, los mismos se van impactando en la previsualización del mapa.
...
En una segunda (y tercera y cuarta) instancia, el usuario cuenta con la posibilidad de cargar sus propias bases de datos con información geográfica. 

```{r out.width="80%", out.height= "50%"}
knitr::include_graphics("imagenes/mapear1.png")
```

Al trabajar con datos de estas características se pueden mapear distintas geometrías, así como utilizar diferentes formatos de archivo: 

+ **Geometrías**: las principales formas de representación de datos espaciales son los puntos, georeferenciados con una latitud y longitud (por ejemplo, las coordenadas de un hotel); las líneas, que representan un sucesión de puntos (por ejemplo, una ruta); y los polígonos, que configuran áreas (como una provincia).

+ **Formatos**: en el caso de puntos es común utilizar archivos con extensión _.xlsx_ o _.csv_ donde se cuenta con una columna para la latitud y otra para la longitud. Para represetnar líneas y polígonos, se suele guardar la información en formatos específicos como _.kml_ o _.geojson_.
